    ...
    <div className="min-h-screen bg-gradient-to-br from-[#3b1f36] to-[#1f2a58] text-white">
      ...
      <header className="flex items-center justify-between p-4 shadow-md">
        ...
        <input
          type="text"
          placeholder="search..."
          className="rounded px-2 py-1 w-64 text-black"
        />
        <nav className="flex gap-4 items-center">
          <a href="#" className="hover:underline">Home</a>
          <a href="#" className="hover:underline">Explore</a>
          <button className="bg-white text-black px-4 py-1 rounded">Wallet</button>
          <div className="w-8 h-6 bg-purple-600 rounded-full" />
        </nav>
      </header>
      ...
      <div className="flex">
        ...
        <aside className="w-64 p-4 text-left">
          ...
          {['Status', 'Price', 'Collections', 'Chains', 'Categories'].map((f) => (
            <details key={f} className="mb-2">
              ...
              <div className="ml-2 text-sm text-gray-300">Options here</div>
            </details>
          ))}
        </aside>
        ...
        <main className="flex-1 p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {products.map((product) => (
            <div
              key={product.id}
              className="bg-[#1f2430] p-4 rounded-xl shadow-md"
            >
              <img
